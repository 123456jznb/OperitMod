...
class TerminalManager private constructor(
     ...
     * 获取当前选择的终端类型
     */
    private fun getSelectedTerminalType(): TerminalType {
        val typeName = prefs.getString("terminal_engine", "LOCAL") ?: "LOCAL"
        return try {
            TerminalType.valueOf(typeName)
    ...
    private suspend fun getTerminalProvider(): TerminalProvider {
        providerMutex.withLock {
            if (terminalProvider == null) {
                val selectedType = getSelectedTerminalType()
                val provider: TerminalProvider = when {
                    // SSH 优先——如果启用 SSH 则使用 SSH
                    sshConfigManager.getConfig() != null && sshConfigManager.isEnabled() -> {
                        Log.d(TAG, "Creating singleton SSH terminal provider")
                        SSHTerminalProvider(context, sshConfigManager.getConfig()!!, this)
                    }
                    // Termux 模式
                    selectedType == TerminalType.TERMUX -> {
                        Log.d(TAG, "Creating singleton Termux terminal provider")
                        TermuxTerminalProvider(context)
                    }
                    // 默认本地终端（proot + Ubuntu）
                    else -> {
