...
class SettingsViewModel(
    ...
    // 终端引擎相关状态
    private val _terminalEngine = MutableStateFlow("LOCAL")
    val terminalEngine = _terminalEngine.asStateFlow()

    private val _termuxDetectionStatus = MutableStateFlow("")
    val termuxDetectionStatus = _termuxDetectionStatus.asStateFlow()
    ...
    init {
        ...
        loadTerminalEngineSetting()
        updateTermuxDetectionStatus()
    }
    ...
    private fun loadTerminalEngineSetting() {
        val engine = prefs.getString("terminal_engine", "LOCAL") ?: "LOCAL"
        _terminalEngine.value = engine
    }

    fun setTerminalEngine(engine: String) {
        prefs.edit().putString("terminal_engine", engine).apply()
        _terminalEngine.value = engine
        // 切换后更新检测状态
        updateTermuxDetectionStatus()
    }

    fun updateTermuxDetectionStatus() {
        val app = getApplication<Application>()
        if (_terminalEngine.value != "TERMUX") {
            // 非 Termux 模式，显示当前引擎
            _termuxDetectionStatus.value = app.getString(
                com.ai.assistance.operit.terminal.R.string.terminal_engine_current,
                if (_terminalEngine.value == "LOCAL") "Operit 内置终端" else _terminalEngine.value
            )
            return
        }

        // Termux 模式，检测安装状态
        val installed = TermuxUtils.detectInstalledTermux(app)
        _termuxDetectionStatus.value = if (installed != null) {
            val name = if (installed == TermuxUtils.TERMUX_PACKAGE) "Termux" else "ZeroTermux"
            app.getString(com.ai.assistance.operit.terminal.R.string.terminal_engine_termux_detected, name)
        } else {
            app.getString(com.ai.assistance.operit.terminal.R.string.terminal_engine_termux_not_found)
        }
    }
} 
